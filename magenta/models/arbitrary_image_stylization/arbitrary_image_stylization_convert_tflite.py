...
flags = tf.flags
flags.DEFINE_float('alpha', 0.25, 'Number of Filters Multiplier the transform model.')
flags.DEFINE_string('checkpoint', None, 'Checkpoint to load the model from')
flags.DEFINE_string('output_dir', None, 'Directory to export TensorFlow Lite models.')
flags.DEFINE_bool('quantize', False, 'Whether to do quantize the TensorFlow Lite models.')
flags.DEFINE_string('style_dataset_file', None, 'Style dataset file used for quantization.')
flags.DEFINE_string('imagenet_data_dir', None,
                    'Path to the ImageNet data used for quantization.')
flags.DEFINE_integer('image_size', 384,
...
def export_to_saved_model(checkpoint, alpha):
  ...
  Args:
    checkpoint: str, path to the checkpoint file.
    alpha: Number of Filters Multiplier of the transform model.

  Returns: (str, str)
     Path to the exported style predict and style transform SavedModel.
  """
  ...
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    style_image_tensor = tf.placeholder(tf.float32,
                                        shape=[None, None, None, 3],
                                        name='style_image')

    # Defines place holder for the content image.
    content_image_tensor = tf.placeholder(tf.float32,
                                          shape=[None, None, None, 3],
                                          name='content_image')
    ...
    tf.saved_model.simple_save(
      sess, predict_saved_model_folder,
    ...
    tf.saved_model.simple_save(
      sess, transform_saved_model_folder,
      inputs={content_image_tensor.name: content_image_tensor,
              'style_bottleneck': bottleneck_feat},
      outputs={'stylized_image': stylized_images}
    )
    tf.logging.debug('Export transform SavedModel to', transform_saved_model_folder)
...
def convert_saved_model_to_tflite(saved_model_dir, input_shapes,
  ...
  tf.logging.info('Converted to TF Lite weight quantized model: %s; Size: %d KB.'
                  % (quantized_tflite_file, len(tflite_quantize_model) / 1024))


def convert_saved_model_to_int8_tflite(saved_model_dir, input_shapes,
                                       representative_dataset, calibrated_tflite_file):
  """Convert SavedModel to int8 quantized TF Lite format.

  Args:
    saved_model_dir: str, path to the SavedModel directory.
    input_shapes: dict, input shapes of the SavedModel.
    representative_dataset: function, generator function to feed to TFLiteConverter
    alpha: Number of Filters Multiplier of the transform model.
  ...
  tf.logging.info('Converted to TF Lite full integer quantized model: %s; Size: %d KB.'
...
def filter_function(image):
  """Generate filtering function that only let the RGB images go through.

  Args:
    image: Tensor, the image tensor to be checked.

  Returns: func
     A filter function to use with tf.data.Dataset.filter() .
  """
...
def get_calibration_dataset(image_size, style_dataset_file, imagenet_data_dir):
  ...
  Returns: (str, str)
     Path to the style dataset and content dataset for calibration.
  """
  ...
  style_dataset = style_dataset.filter(filter_function)
  ...
  content_dataset = content_dataset.map(parse_function(image_size, 'image/encoded'))
  content_dataset = content_dataset.filter(filter_function)
...
def predict_model_gen(session, style_dataset, sample_count):
  ...
  Args:
    session: tf.Session, the session that contains subgraph to load the traning dataset
    style_dataset: tf.data.Dataset that contains training style images.
    sample_count: int, number of sample to create.

  Returns: (str, str)
     A generator function to use as representative dataset for TFLiteConverter.
  """
  def generator():
...
def calculate_style_bottleneck(session,
  ...
  Returns: [[[int]]]
     Value of style bottleneck for the style images being processed.
  """
  ...
  bottleneck_tensor_name = loaded.\
    signature_def["serving_default"].\
    outputs['style_bottleneck'].\
    name
  ...
  for index in range(np.math.ceil(min_sample_count / batch_size)):
    style_image = session.run(next_element)

    style_bottleneck_batch = session.run(bottleneck_tensor_name, feed_dict={
      'style_image:0': style_image
    })
    if styles_bottleneck is None:
      styles_bottleneck = style_bottleneck_batch
    else:
      styles_bottleneck = np.append(styles_bottleneck, style_bottleneck_batch, axis=0)
...
def transform_model_gen(session,
                        predict_saved_model,
                        style_dataset,
                        content_dataset,
                        sample_count):
  """Create a generator function that emits content images and style bottlenecks.
  ...
  # Calculate style bottleneck in advance to use to create representative dataset
  ...
  def generator():
    ...
    for index in range(sample_count):
      ...
      style_bottleneck_input = np.expand_dims(style_bottleneck_list[index], axis=0)
      index += 1
...
def main(unused_argv=None):
  ...
  # Load model and weights from trained checkpoint, and export to SavedModel format
  ...
  predict_weight_quantized_tflite_file = os.path.join(output_dir, 'style_predict_quantized.tflite')
  predict_weight_calibrated_tflite_file = os.path.join(output_dir, 'style_predict_calibrated.tflite')
  tranform_float_tflite_file = os.path.join(output_dir, 'style_transform.tflite')
  tranform_weight_quantized_tflite_file = os.path.join(output_dir, 'style_transform_quantized.tflite')
  tranform_weight_calibrated_tflite_file = os.path.join(output_dir, 'style_transform_calibrated.tflite')
  ...
  convert_saved_model_to_tflite(
    ...
    input_shapes={"style_image": [None, FLAGS.image_size, FLAGS.image_size, 3]},
  ...
  convert_saved_model_to_tflite(
    ...
    input_shapes={"content_image": [None, FLAGS.image_size, FLAGS.image_size, 3]},
    float_tflite_file=tranform_float_tflite_file,
    quantized_tflite_file=tranform_weight_quantized_tflite_file
  )

  if (FLAGS.style_dataset_file is not None) and (FLAGS.imagenet_data_dir is not None):
...
def console_entry_point():
  if FLAGS.quantize and \
    ((FLAGS.style_dataset_file is None) or (FLAGS.imagenet_data_dir is None)):
    raise ValueError('style_dataset_file and imagenet_data_dir are required for quantization.')
