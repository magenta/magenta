...
tf.app.flags.DEFINE_string('run_dir', '/tmp/lookback_rnn/run1',
...
tf.app.flags.DEFINE_string('output_dir', '/tmp/lookback_rnn_generated',
...
def run_generate(graph, train_dir, output_dir, melody_encoder_decoder,
  ...
  """Generates melodies using the latest checkpoint file in `train_dir`.
  ...
  for i, melody in enumerate(melodies):
    ...
    midi_filename = '%d_sample_%d.mid' % (timestamp, i + 1)
...
def main(_):
  ...
  graph = lookback_rnn_graph.build_graph('generate',
                                         hparams_string,
