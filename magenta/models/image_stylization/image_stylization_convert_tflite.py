...
flags.DEFINE_float('alpha', 1.0, 'Number of Filters Multiplier the model was trained on.')
...
flags.DEFINE_bool('quantize', False, 'Whether to do quantize the TensorFlow Lite model.')
flags.DEFINE_integer('image_size', 384,
                     'Default input image size for the generated TensorFlow Lite model.')
FLAGS = flags.FLAGS
...
def _convert_to_tflite(saved_model_dir, num_styles, image_size, quantize, output_model):
  ...
  tf.logging.info('Converted to TF Lite model: %s; Size: %d KB.' % (output_model, len(tflite_model) / 1024))
...
def main(unused_argv=None):
  ...
  _convert_to_tflite(
    saved_model_dir, FLAGS.num_styles,
    FLAGS.image_size, FLAGS.quantize, os.path.expanduser(FLAGS.output_model))
