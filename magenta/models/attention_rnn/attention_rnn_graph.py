...
"""Builds the attention RNN model graph."""
...
class AttentionCellWrapper(tf.nn.rnn_cell.RNNCell):
  ...
  def _attention(self, state, attn_states):
    with tf.variable_scope("Attention"):
      ...
      attn_softmax = tf.nn.softmax(attn)
      attn_softmax = tf.expand_dims(attn_softmax, 2)
      return tf.reduce_sum(attn_states * attn_softmax, 1)
