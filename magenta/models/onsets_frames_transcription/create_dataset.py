...
"""Create the recordio files necessary for training onset and frames."""
...
def generate_train_set():
  ...
  with tf.python_io.TFRecordWriter(train_output_name) as writer:
    for pair in train_file_pairs:
      ...
      samples = audio_io.wav_data_to_samples(wav_data, 16000)
      ...
      splits = find_split_points(ns, samples, 16000,
      ...
      for start, end in zip(splits[:-1], splits[1:]):
        ...
        new_wav_data = audio_io.crop_wav_data(wav_data, 16000,
                                              start, end - start)
