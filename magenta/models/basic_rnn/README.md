...
This model takes monophonic melodies, meaning one note at a time. convert_sequences_to_melodies.py saves tracks from MIDI data that satisfy these requirements: the track should be monophonic, should be at least 7 measures long, and  should have at least 5 unique pitches (with octave equivalence). Many MIDI files will not contain any tracks that make it into the melody dataset, so its good to start with a large MIDI collection.

### How to Use

First follow the instructions in our top level README.md for building models and creating a dataset.
