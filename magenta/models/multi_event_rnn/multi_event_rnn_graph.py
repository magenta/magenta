...
class Graph(object):
  ...
  def __init__(self, examples):
    ...
    self.train_itr = multi_event_rnn_lib.TFRecordDurationAndPitchIterator(
    ...
    self.valid_itr = multi_event_rnn_lib.TFRecordDurationAndPitchIterator(
    ...
    n_duration_symbols = len(multi_event_rnn_lib.TIME_CLASSES)
    ...
    if rnn_type == 'lstm':
      rnn_fork = multi_event_rnn_lib.lstm_fork
      rnn = multi_event_rnn_lib.lstm
      self.rnn_dim = 2 * h_dim
    elif rnn_type == 'gru':
      rnn_fork = multi_event_rnn_lib.gru_fork
      rnn = multi_event_rnn_lib.gru
      self.rnn_dim = h_dim
    else:
    ...
    h1_f = multi_event_rnn_lib.scan(step, [scan_inp], [self.init_h1])
    h1 = h1_f
    self.final_h1 = multi_event_rnn_lib.ni(h1, -1)

    target_note_embed = multi_event_rnn_lib.multiembedding(
        self.note_target, n_note_symbols, note_embed_dim, random_state,
        name=name_note_emb, share_all=share_all_embeddings)
    target_note_masked = multi_event_rnn_lib.automask(
        target_note_embed, self.n_notes)
    target_duration_embed = multi_event_rnn_lib.multiembedding(
        self.duration_target, n_duration_symbols, duration_embed_dim,
        random_state, name=name_dur_emb, share_all=share_all_embeddings)
    target_duration_masked = multi_event_rnn_lib.automask(
    ...
    for i in range(self.n_notes):
      note_pred = multi_event_rnn_lib.linear(
          [
          ...
          name=name_note)
      duration_pred = multi_event_rnn_lib.linear(
          [
              h1[:, :, :h_dim], scan_inp, target_note_masked[i],
              target_duration_masked[i]
          ], [
          ...
          name=name_dur)
      n = multi_event_rnn_lib.categorical_crossentropy(
          multi_event_rnn_lib.softmax(note_pred), self.note_target[:, :, i])
      d = multi_event_rnn_lib.categorical_crossentropy(
          multi_event_rnn_lib.softmax(duration_pred),
