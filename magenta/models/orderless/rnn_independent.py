...
def trainingLoss(inputs, targets, lengths, opts):
	...
	# Compute sigmoid cross entropy loss, sum across time and batch
	sce = tf.nn.sigmoid_cross_entropy_with_logits(logits_flat, targets_flat)
	loss = tf.reduce_sum(mask_flat * sce) / num_logits
...
def doTraining(opts):
	...
	trainIter = trainingIteration(loss, opts)

	# sv = tf.train.Supervisor(logdir=train_dir, save_model_secs=30,
	# 					   global_step=global_step)
	sv = tf.train.Supervisor(global_step=global_step)

	with sv.managed_session() as sess:
		global_step_ = sess.run(trainIter.global_step)
