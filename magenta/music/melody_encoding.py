...
MelodyEventOneHotEncoding is an encoding.OneHotEncoding that specifies a one-
...
MelodyEncoderDecoder is an encoding.EventSequenceEncoderDecoder that does the
actual translation between melodies and model data.

Use MelodyEncoderDecoder.encode or MelodyEncoderDecoder.squash_and_encode to
convert a Melody object to a tf.train.SequenceExample of inputs and labels.
These SequenceExamples are fed into the model during training and evaluation.
...
OneHotMelodyEncoderDecoder, LookbackMelodyEncoderDecoder, and
KeyMelodyEncoderDecoder are subclasses of MelodyEncoderDecoder that use the
...
class MelodyEventOneHotEncoding(encoding.OneHotEncoding):
  ...
  def __init__(self, min_note=48, max_note=84):
    """Initializes a MelodyEventOneHotEncoding object.
...
class KeyMelodyEncoding(encoding.EventSequenceEncoding):
  ...
  Args:
    lookback_distances: A list of step intervals to look back in history to
       ...
       If None, use default lookback distances.
...
class MelodyEncoderDecoder(encoding.EventSequenceEncoderDecoder):
  """An encoder/decoder for translating between melodies and model data.
  ...
  Attributes:
    ...
    no_event_label: The class label that represents a MELODY_NO_EVENT Melody
        event.
  """

  def __init__(self, melody_encoding_fn, min_note=48, max_note=84,
               transpose_to_key=0):
    ...
    Args:
      melody_encoding_fn: A function that takes `min_note` and `max_note` as
          input and returns an EventSequenceEncoding object that specifies the
          translation between melodies and model input & output.
      min_note: The minimum midi pitch the encoded melodies can have.
    ...
    super(MelodyEncoderDecoder, self).__init__(
        melody_encoding_fn(min_note, max_note))
  ...
  def no_event_label(self):
    """The class label that represents a MELODY_NO_EVENT Melody event.

    Returns:
      An int, the class label that represents a MELODY_NO_EVENT.
    """
    melody = melodies_lib.Melody([MELODY_NO_EVENT])
    return self._encoding.events_to_label(melody, 0)
  ...
  def squash_and_encode(self, melody):
    ...
    return self.encode(melody)


def one_hot_melody_encoding(min_note, max_note):
  """Create a OneHotEventSequenceEncoding object for melodies.
  ...
  Returns:
    A OneHotEventSequenceEncoding for melodies.
  """
  return encoding.OneHotEventSequenceEncoding(
      MelodyEventOneHotEncoding(min_note, max_note))


def lookback_melody_encoding(min_note, max_note, lookback_distances=None,
                             binary_counter_bits=5):
  """Create a LookbackEventSequenceEncoding object for melodies.
  ...
  Returns:
    A LookbackEventSequenceEncoding for melodies.
  """
  return encoding.LookbackEventSequenceEncoding(
      MelodyEventOneHotEncoding(min_note, max_note), lookback_distances,
      binary_counter_bits)


class OneHotMelodyEncoderDecoder(MelodyEncoderDecoder):
  """A MelodyEncoderDecoder that uses a one-hot melody encoding."""

  def __init__(self, min_note=48, max_note=84, transpose_to_key=0):
    """Initialize a OneHotMelodyEncoderDecoder object.
    ...
    Args:
      ...
      transpose_to_key: The key that encoded melodies will be transposed into,
          or None if it should not be transposed.
    """
    super(OneHotMelodyEncoderDecoder, self).__init__(
        one_hot_melody_encoding, min_note, max_note, transpose_to_key)


class LookbackMelodyEncoderDecoder(MelodyEncoderDecoder):
  """A MelodyEncoderDecoder that uses a one-hot encoding with lookback."""

  def __init__(self, lookback_distances=None, binary_counter_bits=5,
               min_note=48, max_note=84, transpose_to_key=0):
    """Initialize a LookbackMelodyEncoderDecoder object.
    ...
    super(LookbackMelodyEncoderDecoder, self).__init__(
        partial(lookback_melody_encoding,
                lookback_distances=lookback_distances,
                binary_counter_bits=binary_counter_bits),
...
class KeyMelodyEncoderDecoder(MelodyEncoderDecoder):
  ...
  def __init__(self, lookback_distances=None, binary_counter_bits=7,
               min_note=48, max_note=84, transpose_to_key=0):
    """Initialize a KeyMelodyEncoderDecoder object.
