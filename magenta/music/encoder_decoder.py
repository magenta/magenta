...
ModuloEventSequenceEncoderDecoder is an EventSequenceEncoderDecoder that uses
Modulo Encoding for individual note-on events. This encoding can be
used in place of OneHotEventSequenceEncoderDecoder in models that only use
note-on and note-off events. The input vectors are modulo-12 encodings
of the most recent note event. The output labels are one-hot encodings of the
next note event.
...
place of OneHotEventSequenceEncoderDecoder in models that use time-shift and/or
velocity events in addition to note-on/note-off events. The input vectors are
modulo-12 encodings of the most recent event. The output labels are one-hot
encodings of the next event.
...
class ModuloEventSequenceEncoderDecoder(EventSequenceEncoderDecoder):
  """An EventSequenceEncoderDecoder producing modulo encodings for note events.

  An EventSequenceEncoderDecoder that produces modulo encodings for note events
  by first encoding them using a one-hot encoder. This can be used for
  non-performance models, such as melody_rnn model, to make them use modulo
  encoding. The only method of this class that is different from that of
  OneHotEventSequenceEncoderDecoder is events_to_inputs().
  """

  def __init__(self, one_hot_encoding):
    """Initialize a ModuloEventSequenceEncoderDecoder object.
  ...
  @property
  def input_size(self):
    # Use 6 floats to represent the following:
    # no_event, off_event, octave_cos, octave_sin, note_cos, note_sin
    return 6
  ...
  def events_to_input(self, events, position):
    ...
    Returns a modulo encoding for the given position in the event sequence, by
    modulo-encoding the output of the one-hot-encoder applied to that event
    position.
    ...
    """
    input_ = [0.0] * self.input_size
    # First find the one-hot-encoding of the event
    # (i.e., the position of 1 in the one-hot vector), and then use it to
    # generate a modulo encoding for the event.
    label = self._one_hot_encoding.encode_event(events[position])
    if label == 0:
      input_[0] = 1.0  # no_event
    elif label == 1:
      input_[1] = 1.0  # off_event
    else:
      # Note: here the assumption is that NUM_SPECIAL_MELODY_EVENTS is 2, and
      # that is why in the above, the values of 0 and 1 are assumed to carry
      # no_event and off_event information, respectively, and any other value
      # is assumed to carry an on_event information when adjusted for the first
      # two values. The following code needs to be modified if in the future
      # these assumptions do not hold.
      label -= constants.NUM_SPECIAL_MELODY_EVENTS

      # Encode the octave of the note.
      angle = (float(label) * math.pi) / 72.0  # 12 octaves, 144 notes
      input_[2] = math.cos(angle)
      input_[3] = math.sin(angle)

      # Encode the note itself, using modulo-12 encoding.
      angle = (float(label) * math.pi) / 6.0  # 12 notes per octave
      input_[4] = math.cos(angle)
      input_[5] = math.sin(angle)
    return input_
