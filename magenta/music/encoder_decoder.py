  ...
  *conditional* event sequences, where there is both a control event sequence
  ...
  def get_inputs_batch(self, control_event_sequences, target_event_sequences,
    ...
    Args:
      ...
      target_event_sequences: A list of list-like target event sequences. Should
          be the same length as `control_event_sequences`.
      full_length: If True, the inputs batch will be for the full length of
    ...
    Raises:
      ValueError: If the number of control and target event sequences are not
          the same, or if one of the pairs of sequences has control not longer
          than target.
    """
    if len(control_event_sequences) != len(target_event_sequences):
      raise ValueError('must have the same number of control and target '
                       'event sequences (%d control event sequences but %d '
                       'target event sequences)' % (
                           len(control_event_sequences),
                           len(target_event_sequences)))

    inputs_batch = []
    for control_events, target_events in zip(control_event_sequences,
